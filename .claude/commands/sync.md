...
```
@jogi/tables dirty (N files, M LOC). Commit + push? (y / n / w)
...
INSTALLED_SHA=$(grep -oE 'jogi-tables#[a-f0-9]{40}' ../jogi/package-lock.json | head -1 | grep -oE '[a-f0-9]{40}')
```
...
```bash
cd ../jogi
npm install @jogi/tables@github:luvidal/jogi-tables#$LOCAL_SHA
npx tsc --noEmit
```
